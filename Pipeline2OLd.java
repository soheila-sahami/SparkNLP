...
public class Pipeline2OLd {
	...
	static boolean beSegmentized = false;
	...
	private final static Logger logger = Logger.getLogger(Pipeline2OLd.class.getName());

	public static void main(String[] args) {
		SparkConf conf = new SparkConf().setAppName("SegmentizerTokenizer");
		...
		// JavaPairRDD<Long, Tuple2<String, String>> documents;
		// if (metaDataIncluded) {
		// documents = texts.mapToPair(new ReadDocumentMetaData()).filter(doc ->doc._1!=
		// -1);
		// } else {
		// documents = texts.mapToPair(new ReadDocuments());
		// }
		...
		if (beCleaned) {
			...
			if (inputType.equals("raw-text")) {
				...
				//
				// @Override
				// public Iterator<Tuple2<Long, String>> call(Tuple2<Long, String> line) throws
				// Exception {
				// return checkRawtextFile(line, replaceStrings, replacements, exchangeOutput,
				// filterMap);
				// }
				// }).filter(line -> line._2.length() > 0);
				// if (saveMiddleResults && saveWithSources) {
				// cleanedLines.mapToPair(new ExtrcactSourceID(offset)).groupByKey()
				// .mapToPair(new ConcatSortDocs(seperator)).join(sourceIndex)
				// .map(x -> x._2._2 + seperator + x._2._1).saveAsTextFile(outputPath +
				// "/Cleaned/");
				// logger.info("Done! Cleaner finished. ");
				// } else if (saveMiddleResults) {
				// cleanedLines.sortByKey().saveAsTextFile(outputPath + "/Cleaned/");
				...
				// return checkTabFile(line, inputColumn, replaceStrings, replacements,
				// exchangeOutput, filterMap);
				// }
				// }).filter(line -> line._2.length() > 0);
				// if (saveMiddleResults && saveWithSources) {
				// cleanedLines.mapToPair(new ExtrcactSourceID(offset)).groupByKey()
...
//				logger.info("==================================================================================");
	...
	/*
	 * public static Iterator<Tuple2<Long, String>> checkTabFile(Tuple2<Long,
	 * String> line, int inputColumn, Boolean replaceStrings, HashMap<String,
	 * String> replacements, Boolean exchangeOutput, HashMap<Integer,
	 * SentenceFilter> filterMap) { List<Tuple2<Long, String>> finalOutput = new
	 * ArrayList<Tuple2<Long, String>>(); boolean isValid = true; ArrayList<String>
	 * output = new ArrayList<String>(); String[] lineArray = line._2.split("\t");
	 * // replace special characters if (inputColumn < lineArray.length &
	 * replaceStrings & replacements.size() > 0) { lineArray[inputColumn] = new
	 * StringReplacements(replacements).replaceEntities(lineArray[inputColumn]); }
	 * // sequential filter checks isValid = true; Iterator<Integer> iter =
	 * filterMap.keySet().iterator(); while (iter.hasNext()) { SentenceFilter filter
	 * = filterMap.get(iter.next()); if (inputColumn >= lineArray.length ||
	 * !filter.sentenceIsValid(lineArray[inputColumn])) { isValid = false; if
	 * (exchangeOutput) { // write ill-formed sentences String outputLine = ""; for
	 * (int i = 0; i < lineArray.length; i++) outputLine += lineArray[i] + "\t"; if
	 * (outputLine.length() > 0) outputLine = outputLine.substring(0,
	 * outputLine.length() - 1); outputLine += "\tRule: " + filter.getFilterID() +
	 * " " + filter.getFilterDescription() + "\n"; output.add(outputLine); } break;
	 * } } // write output if (isValid && !exchangeOutput) { String outputLine = "";
	 * for (int i = 0; i < lineArray.length; i++) outputLine += lineArray[i] + "\t";
	 * if (outputLine.length() > 0) outputLine = outputLine.substring(0,
	 * outputLine.length() - 1); outputLine += "\n"; output.add(outputLine); } for
	 * (int i = 0; i < output.size(); i++) finalOutput.add(new Tuple2<Long,
	 * String>(line._1, output.get(i)));
	 * 
	 * return (finalOutput.iterator()); }
	 * 
	 * public static Iterator<Tuple2<Long, String>> checkRawtextFile(Tuple2<Long,
	 * String> lineMetadata, Boolean replaceStrings, HashMap<String, String>
	 * replacements, Boolean exchangeOutput, HashMap<Integer, SentenceFilter>
	 * filterMap) { List<Tuple2<Long, String>> finalOutput = new
	 * ArrayList<Tuple2<Long, String>>(); boolean isValid = true; ArrayList<String>
	 * output = new ArrayList<String>(); String line = lineMetadata._2; // replace
	 * special characters if (replaceStrings & replacements.size() > 0) { line = new
	 * StringReplacements(replacements).replaceEntities(line); } // sequential
	 * filter checks isValid = true; Iterator<Integer> iter =
	 * filterMap.keySet().iterator(); while (iter.hasNext()) { SentenceFilter filter
	 * = filterMap.get(iter.next()); if (!filter.sentenceIsValid(line)) { isValid =
	 * false; if (exchangeOutput) // write ill-formed sentences output.add(line +
	 * "\tRule: " + filter.getFilterID() + " " + filter.getFilterDescription());
	 * break; } } // write well-formed sentences if (isValid && !exchangeOutput)
	 * output.add(line); for (int i = 0; i < output.size(); i++) finalOutput.add(new
	 * Tuple2<Long, String>(lineMetadata._1, output.get(i)));
	 * 
	 * return (finalOutput.iterator()); }
	 */
}
