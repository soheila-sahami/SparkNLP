...
public class PipelineLocalOld {
	...
	static boolean metaDataIncluded = false;
	static boolean beSorted = false;
	...
	private final static Logger logger = Logger.getLogger(PipelineLocalOld.class.getName());
	...
	public static void main(String[] args) {
		...
		logger.info("Done! loading entence boundaries list.");
		...
		JavaPairRDD<Long, String> sourceIndex = documents.flatMapToPair(new CreateDocumentSourceIndex()).cache();
		sourceIndex.saveAsTextFile(outputPath + "SourceIndex");

		JavaPairRDD<Long, String> sentences = documents.map((x) -> new Tuple2<Long, String>(x._1, x._2._2))
				.flatMapToPair(new SegmentizeDocument(preList, boundaries, compiledPostRuleSet, compiledPreRulesSet,
						postList, longestAbbrevation, encodingText, offset));

		if (saveMiddleResults && saveWithSources) {
			sentences.mapToPair(new ExtrcactSourceID(offset)).groupByKey().mapToPair(new ConcatAndSortDocs(seperator))
					.join(sourceIndex).map(x -> x._2._2 + seperator + x._2._1)
			...
			if (saveMiddleResults) {
				if (beSorted) {
					sentences.sortByKey().saveAsTextFile(outputPath + "/Segmentized");
				} else {
					sentences.saveAsTextFile(outputPath + "/Segmentized");
				}
				logger.info("Done! Sentence Segmentizer finished. ");
			}
		}
		...
		///// Cleaner
		JavaPairRDD<Long, String> cleanedLines = null;
		if (beCleaned) {
			// load general rules
			...
			if (inputType.equals("raw-text")) {
				...
				cleanedLines = sentences.flatMapToPair(new PairFlatMapFunction<Tuple2<Long, String>, Long, String>() {
				...
				if (saveMiddleResults && saveWithSources) {
					...
					if (beSorted) {
						...
						cleanedLines.saveAsTextFile(outputPath + "/Cleaned/");
					}
				}
				logger.info("Done! Cleaner finished. ");
		...
		// tokenization
		if (beTokeniized) {
			JavaPairRDD<Long, String> tokenizedSentences;
			if (beCleaned) {
				tokenizedSentences = cleanedLines
			...
			if (saveWithSources) {
				...
				tokenizedSentences.saveAsTextFile(outputPath + "/Tokenised");
			}
			logger.info("Done! Tokeniser finished. ");
		}
	...
	private static void init(final String[] args) {
		Getopt g = new Getopt("SentenceSegmentizer", args, ":hi:o:r:e:x:mtcsdbl:n:p:gu");
		int c;
		while ((c = g.getopt()) != -1) {
			switch (c) {
			case 'h':
	...
	public static void printHelp() {
		...
		logger.info("s\t Sort the final output. default: false");
		...
		logger.info("g\t Ehange: write the ill-formed sentences to output (+triggered rule)");
	}
