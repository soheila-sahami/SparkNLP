...
public class SimpleSentenceFilter implements SentenceFilter, Serializable {
	 ...
	 * @param verbose
	 */
	public SimpleSentenceFilter(String filterDescription, String patternString, int minLength, int maxLength, String hexPatternString, boolean verbose) {
	 ...
	 * @param hexPattern
	 */
	public SimpleSentenceFilter(String filterDescription, String patternString, int minLength, int maxLength, String hexPattern) {
	...
	public boolean filterIsValid() {
		if (filterID != -1
				&& (pattern != null || hexPattern != null || minLength != 0 || maxLength != 0 || (replaceCharacterString != null && (replaceRatio != 0 || replaceCount != 0))))
			return true;
		else
			return false;
	}
	...
	public boolean sentenceIsValid(String sentence) {
		...
		if (hexPattern != null) {
			try {
				for (int i = 0; i < sentence.length(); i++) {
					String hexChar = String.format("%x", new BigInteger(1, String.valueOf(sentence.charAt(i)).getBytes("UTF-8")));
		...
		if (failed) {
			hits++;
			if (verbose)
				System.out.println("sentence \"" + sentence + "\" failed test: \"" + this.filterID + " - " + this.filterDescription + "\"");
		}
	...
	public SentenceFilter clone() {
		return new SimpleSentenceFilter(filterDescription, patternString, minLength, maxLength, hexPatternString, verbose);
	}
