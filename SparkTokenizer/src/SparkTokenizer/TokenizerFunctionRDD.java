...
public class TokenizerFunctionRDD implements Function<Tuple2<String, Long>, Tuple2<String, Long>> {
	...
	public TokenizerFunctionRDD(List<String> _abbrevList, List<String> _fixedTokens, List<Tuple2<String, Integer[]>> _tokenisationAction, String[] _evilSentenceEndCharacters,
			String[] _punctuationCharacters) {
		...
		for (int i = 0; i < tokenisationAction.size(); i++) {
			String tmp = tokenisationAction.get(i)._1;
			characters.add(tmp);
	...
	private String execute(String strLine) {
		...
		while (!tokenizedLine.equals(strLine)) {
			tokenizedLine = strLine;
			...
			while (objIter.hasNext()) {
				...
				if (intEnd - intStart > 0) {
					...
					if (!(strWord.equals("%^%") || strWord.equals("%$%") || strWord.equals("%N%") || strWord.equals("%TAB%"))) {
						...
						strTokenisedWord = processPrefix(strTokenisedWord, sentenceEnd);
						strTokenisedWord = processInfix(strTokenisedWord);
			...
			strLine = objBuffer.toString().trim();
		}
		return strLine;
	}
	...
	private String processPrefix(String strWord, boolean sentenceEnd) {
		...
		for (int i = 0; i < characterCount; i++) {
			...
			if (strWord.startsWith(characters.get(i), intOffSet)) {
				int intLengthOfCharacter = getCharacterLength(characters.get(i));
	...
	private String processSuffix(String strWord, boolean sentenceEnd) {
		...
		for (int i = 0; i < characterCount; i++) {
			...
			if (strTMPWord.endsWith(characters.get(i))) {
				...
				int desiredAction = tokenisationAction.get(indesOfCharInActionList(characters.get(i)))._2[whatToDo];
	...
	private String processInfix1(String strWord) {
		...
		for (int i = 0; i < characterCount; i++) {
			...
			int charPos = strTMPWord.indexOf(characters.get(i));
			if (charPos >= 0) {
				if (charPos == strTMPWord.length() - 1 || charPos == 0) {
					if (objBuffer.indexOf(characters.get(i)) < 0) {
				...
				int intLengthOfCharacter = getCharacterLength(characters.get(i));
				// use word mid action
				desiredAction = tokenisationAction.get(indesOfCharInActionList(characters.get(i)))._2[1];
				if (desiredAction == 1) {
					strTMPWord = strTMPWord.substring(charPos + intLengthOfCharacter);
					objBuffer.append(strWord.substring(0, charPos) + " " + characters.get(i));
					objBuffer.append(" ");
				} else if (desiredAction == 0 || desiredAction == 3) {
					// do nothing
					strTMPWord = strTMPWord.substring(charPos + intLengthOfCharacter);
				...
				if (strTMPWord.indexOf(characters.get(i)) >= 0) {
					i--;
					strWord = strTMPWord;
					continue;
				} else {
