...
public class Pipeline {
	...
	public static void main(String[] args) {
		SparkConf conf = new SparkConf().setAppName("NLP Pipeline");
		...
		List<String> postList = readPostBoundaryList(sparkContext, postListFile);
		List<Tuple2<Pattern, Boolean>> compiledPostRuleSet = readPostRulesFile(sparkContext, postRulesFile);
		List<String> preList = readPreListFile(sparkContext, preListFile);
		longestAbbrevationLength = Collections.max(preList, Comparator.comparing(s -> s.length())).length();
		List<Tuple2<Pattern, Boolean>> compiledPreRulesSet = readPreRulesFile(sparkContext, preRulesFile);
		List<String> abbreviationList = readAbbreviationsFile(sparkContext, abbreviationsFile);
		List<String> fixedTokens = readFixedTokensFile(sparkContext, fixedTokensFile);
		List<Tuple2<String, Integer[]>> tokenisationAction = readTokenisationActionFile(sparkContext,
				tokenisationActionFile);
		// read input file
	...
	private static List<Tuple2<String, Integer[]>> readTokenisationActionFile(JavaSparkContext sparkContext,
			String tokenisationActionFile) {
		logger.info("Loading tokenization character actions from: " + tokenisationActionFile + ", encoding "
				+ encodingText);
		...
		return tokenisationAction;
	}

	private static List<String> readFixedTokensFile(JavaSparkContext sparkContext, String fixedTokensFile) {
	...
	private static List<String> readAbbreviationsFile(JavaSparkContext sparkContext, String abbreviationsFile) {
	...
	private static List<Tuple2<Pattern, Boolean>> readPreRulesFile(JavaSparkContext sparkContext, String preRulesFile) {
		...
		return compiledPreRulesSet;
	}

	private static List<String> readPreListFile(JavaSparkContext sparkContext, String preListFile) {
	...
	private static List<Tuple2<Pattern, Boolean>> readPostRulesFile(JavaSparkContext sparkContext,
			String postRulesFile) {
		...
		return compiledPostRuleSet;
	}

	private static List<String> readPostBoundaryList(JavaSparkContext sparkContext, String postListFile) {
